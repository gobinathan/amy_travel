...
if ( $config->max_data_age_days > 0 ) {
	...
	if ( $max_age > $first_hit ) {
		if ( isset( $_POST["confirm"] ) && $_POST["confirm"] == "kill" ) {
			...
			print "done!</p>\n";
			$query = "DELETE FROM `".SlimStat::my_esc( $config->stats )."`";
			...
			<p>If you want to delete all Stat data more than <?php print $config->max_data_age_days; ?> days old (before <?php print date( "j M Y", $max_age ); ?>), you must confirm this decision below.</p>
			<p><strong>This CANNOT be undone.</strong> It is recommended that you save a recently-generated Stat report for posterity.</p>
			<p><label for="confirm">
			<input type="checkbox" id="confirm" name="confirm" value="kill" /> 
			I confirm deletion of all Stat data from before <?php print date( "j M Y", $max_age ); ?>
			</label></p>
		...
		<p>Stat is configured to delete data more than <?php print $config->max_data_age_days; ?> days old, or from before <?php print date( "j M Y", $max_age ); ?>.</p>
