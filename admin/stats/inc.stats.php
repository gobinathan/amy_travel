...
class SlimStatRecord {
	...
	function SlimStatRecord() {
		...
		if ( $this->config->enabled ) {
			...
			if ( $is_ignored ) {
				...
				$query = "INSERT INTO `".SlimStat::my_esc( $this->config->stats )."` ( `";
	...
	function _determine_country( $_ip ) {
		if ( SlimStat::is_ip_to_country_installed( $this->config ) ) {
			...
			$query = "SELECT `country_name` FROM `".SlimStat::my_esc( $this->config->countries )."` WHERE `ip_from` <= ".$ip." AND `ip_to` >= ".$ip;
	...
	function _determine_visit( $_remote_ip, $_browser, $_version, $_platform, $_user_agent ) {
		$query = "SELECT `visit` FROM `".SlimStat::my_esc( $this->config->stats )."`";
		...
		$query = "SELECT MAX(`visit`) FROM `".SlimStat::my_esc( $this->config->stats )."`";
