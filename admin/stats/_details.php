		...
		foreach ( array_keys( $filters ) as $filter ) {
			if ( $filter == "filter_dt_start" ) {
				...
				if ( ( $filter == "filter_remote_ip" || $filter == "filter_domain" ) && $config->whoisurl != "" ) {
					print " <a href=\"".str_replace( '%i', $filters[$filter], $config->whoisurl )."\" title=\"".$config->i18n->link_title( "whois" )."\">?</a>";
				}
				print "</span> <a href=\"?";
...
function total_table( $_col, $_truncate, $_where_clause="", $_filters ) {
	...
	foreach ( $results as $result ) {
		...
		if ( is_array( $_col ) ) {
			...
			$row["last"][] = "<a href=\"".$result[$_col]."\" target=_blank class=\"external\" rel=\"nofollow\" title=\"".$config->i18n->link_title( "external", "FIELD", strtolower( $config->i18n->fields[$_col] ) )."\"><img src=\"external.gif\" width=\"9\" height=\"9\" alt=\"\" /></a>";
		} else {
			$row["first"][] = get_td( array( $_col ), $result, $_col, $_truncate, ( isset( $config->i18n->name_lookups[$_col] ) ) ? $config->i18n->name_lookups[$_col] : null, true );
		}
