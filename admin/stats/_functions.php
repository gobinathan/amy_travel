...
class SlimStat {
	...
	function is_ip_to_country_installed() {
		$config =& SlimStatConfig::get_instance();
		$query = "SELECT * FROM `".SlimStat::my_esc( $config->countries )."` LIMIT 0, 1";
	...
	function _get_table_data( $_col, $_where_clause="", $_group_by, $_order_by, $_filters=array() ) {
		...
		$query .= " FROM `".SlimStat::my_esc( $config->stats )."` WHERE ";
		if ( $_where_clause != "" ) {
	...
	function get_unique_data( $_col, $_where_clause="", $_group_by, $_filters=array() ) {
		...
		foreach ( $results as $result ) {
			$query = "SELECT MIN(`dt`) AS `mindt` FROM `".SlimStat::my_esc( $config->stats )."` WHERE ";
	...
	function get_visit_data( $_filters=array() ) {
		...
		$query = "SELECT `visit`, `remote_ip`, COUNT(`id`) AS `hits`, MIN(`dt`) AS `mindt`, MAX(`dt`) as `maxdt` FROM `".SlimStat::my_esc( $config->stats )."` WHERE ".SlimStat::get_filter_clause( $_filters )." GROUP BY `visit` ORDER BY `hits` DESC, `mindt` DESC, `maxdt` DESC LIMIT 0, ".SlimStat::my_esc( $config->rows );
	...
	function get_pageviews_data( $_where_clause, $_filters=array() ) {
		...
		$query = "SELECT COUNT(`id`) AS `hits` FROM `".SlimStat::my_esc( $config->stats )."` WHERE ";
		if ( $_where_clause != "" ) {
			$query .= $_where_clause." AND ";
		}
	...
	function _count( $_count, $_where_clause="", $_filters=array() ) {
		...
		$query = "SELECT COUNT(".SlimStat::my_esc( $_count ).") FROM `".SlimStat::my_esc( $config->stats )."` WHERE ";
		if ( $_where_clause != "" ) {
			$query .= $_where_clause." AND ";
		}
		$query .= SlimStat::get_filter_clause( $_filters );
	...
	function get_hits_visits_uniques( $_dt_start, $_dt_end=0, $_filters=array() ) {
		...
		if ( !empty( $_filters ) ) {
			...
			$query .= " FROM `".SlimStat::my_esc( $config->dt_table )."`";
			...
			if ( $result = mysql_query( $query ) ) {
				if ( mysql_num_rows( $result ) == 1 ) {
					...
					$query = "INSERT INTO `".SlimStat::my_esc( $config->dt_table )."`";
	...
	function get_first_hit( $_filters=array() ) {
		...
		$query = "SELECT `dt` FROM `".SlimStat::my_esc( $config->stats )."`";
	...
	function get_host( $_ip, $_do_lookup ) {
		...
		$query = "SELECT `remote_addr` FROM `".SlimStat::my_esc( $config->stats )."` WHERE `remote_ip`='".SlimStat::my_esc( $_ip )."' AND `remote_addr`<>'' LIMIT 1";
		$host = "";
		if ( $result = mysql_query( $query ) ) {
		...
		if ( $host != "" ) {
			$query = "UPDATE `".SlimStat::my_esc( $config->stats )."` SET `remote_addr`='".SlimStat::my_esc( $host )."' WHERE `remote_ip`='".SlimStat::my_esc( $_ip )."'";
