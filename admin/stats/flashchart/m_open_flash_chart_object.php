...
function _ofc( $width, $height, $url, $use_swfobject, $base )
{
    ...
    if( !isset( $open_flash_chart_seqno ) )
    {
        ...
        $out[] = '<script type="text/javascript" src="../js/swfobject.js"></script>';
    }
    else
    {
	...
	$out[] = 'var so = new SWFObject("stats/flashchart/open-flash-chart.swf", "'. $obj_id .'", "'. $width . '", "' . $height . '", "9", "#FFFFFF");';
    ...
    $out[] = '<param name="movie" value="stats/flashchart/open-flash-chart.swf?width='. $width .'&height='. $height . '&data='. $url .'" />';
    $out[] = '<param name="quality" value="high" />';
    $out[] = '<param name="bgcolor" value="#FFFFFF" />';
    $out[] = '<embed src="stats/flashchart/open-flash-chart.swf?data=' . $url .'" quality="high" bgcolor="#FFFFFF" width="'. $width .'" height="'. $height .'" name="'. $obj_id .'" align="middle" allowScriptAccess="sameDomain" ';
