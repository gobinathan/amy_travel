...
if ( SlimStat::get_first_hit() == 0 ) {
	?>
	<p>You'll need to wait for people to start visiting your site. Then the boxes below will start to fill up.</p>
	...
	if ( $config->show_modules["summary"] ) {
		...
		<div class="module">
			...
			<div><table border="0" cellspacing="0" cellpadding="0">
				...
				?>
				<tr>
					<td><a href="?show=today" title="<?php print $config->i18n->link_title( 'details_dt', 'DT', $config->i18n->date_periods['today'] ); ?>"><?php print SlimStat::date_period_label( $svr_dt, $svr_dt_end ); ?></a></td>
				...
				?>
				<tr>
					<td><a href="?filter_dt_start=<?php print $dt_start; ?>&amp;filter_dt_end=<?php print $dt - 1; ?>" title="<?php print $config->i18n->link_title( 'details_dt', 'DT', $config->i18n->date_periods['yesterday'] ); ?>"><?php print ucfirst( $config->i18n->date_periods["yesterday"] ); ?></a></td>
					<td class="last"><?php print $hvu["hits"]; ?></td>
					<?php if ( $config->show_visits ) { ?><td class="last"><?php print $hvu["visits"]; ?></td><?php } ?>
					<?php if ( $config->show_uniques ) { ?><td class="last"><?php print $hvu["uniques"]; ?></td><?php } ?>
				</tr>
				...
				?>
				<tr>
					<td><a href="?show=thisweek" title="<?php print $config->i18n->link_title( 'details_dt', 'DT', $config->i18n->date_periods['this_week'] ); ?>"><?php print ucfirst( $config->i18n->date_periods["this_week"] ); ?></a></td>
					<td class="last"><?php print $hvu["hits"]; ?></td>
					<?php if ( $config->show_visits ) { ?><td class="last"><?php print $hvu["visits"]; ?></td><?php } ?>
					<?php if ( $config->show_uniques ) { ?><td class="last"><?php print $hvu["uniques"]; ?></td><?php } ?>
				</tr>
				...
				?>
				<tr>
					<td><a href="?filter_dt_start=<?php print $dt_start; ?>&amp;filter_dt_end=<?php print $dt - 1; ?>" title="<?php print $config->i18n->link_title( 'details_dt', 'DT', $config->i18n->date_periods['last_week'] ); ?>"><?php print ucfirst( $config->i18n->date_periods["last_week"] ); ?></a></td>
					<td class="last"><?php print $hvu["hits"]; ?></td>
					<?php if ( $config->show_visits ) { ?><td class="last"><?php print $hvu["visits"]; ?></td><?php } ?>
					<?php if ( $config->show_uniques ) { ?><td class="last"><?php print $hvu["uniques"]; ?></td><?php } ?>
				</tr>
				...
				?>
				<tr>
					<td><a href="?show=thismonth" title="<?php print $config->i18n->link_title( 'details_dt', 'DT', $config->i18n->date_periods['this_month'] ); ?>"><?php print ucfirst( $config->i18n->date_periods["this_month"] ); ?></a></td>
					<td class="last"><?php print $hvu["hits"]; ?></td>
					<?php if ( $config->show_visits ) { ?><td class="last"><?php print $hvu["visits"]; ?></td><?php } ?>
					<?php if ( $config->show_uniques ) { ?><td class="last"><?php print $hvu["uniques"]; ?></td><?php } ?>
				</tr>
				...
				?>
				<tr>
					<td><a href="?filter_dt_start=<?php print $dt_start; ?>&amp;filter_dt_end=<?php print $dt - 1; ?>" title="<?php print $config->i18n->link_title( 'details_dt', 'DT', $config->i18n->date_periods['last_month'] ); ?>"><?php print ucfirst( $config->i18n->date_periods["last_month"] ); ?></a></td>
					<td class="last"><?php print $hvu["hits"]; ?></td>
					<?php if ( $config->show_visits ) { ?><td class="last"><?php print $hvu["visits"]; ?></td><?php } ?>
					<?php if ( $config->show_uniques ) { ?><td class="last"><?php print $hvu["uniques"]; ?></td><?php } ?>
				</tr>
				...
				?>
				<tr>
					<td><a href="?show=details" title="<?php print $config->i18n->link_title( 'details_all' ); ?>"><?php print $config->i18n->since." ".SlimStat::date_label( SlimStat::get_first_hit( array() ) ); ?></a></td>
					<td class="last"><?php print $hvu["hits"]; ?></td>
					<?php if ( $config->show_visits ) { ?><td class="last"><?php print $hvu["visits"]; ?></td><?php } ?>
					<?php if ( $config->show_uniques ) { ?><td class="last"><?php print $hvu["uniques"]; ?></td><?php } ?>
				</tr>
	...
	if ( $config->show_modules["unique_domain"] ) {
		print SlimStat::render_module(
			$config->i18n->module_titles["unique_domain"]." <span><a href=\"rss.php?domain\">".$config->i18n->feed."</a></span>",
			unique_table(
				array( "href" => "referer", "title" => "resource", "display" => "domain" ),
				$config->truncate,
	...
	if ( $config->show_modules["unique_resource"] ) {
		print SlimStat::render_module(
			$config->i18n->module_titles["unique_resource"]." <span><a href=\"rss.php?resource\">".$config->i18n->feed."</a></span>",
			unique_table(
				array( "href" => "resource", "title" => ( $config->show_hostnames ) ? "remote_addr" : "remote_ip", "display" => "resource" ),
				$config->truncate_medium,
				"dt > ".$config->recent_threshold
...
function recent_table( $_cols, $_truncate, $_where_clause="", $_filters=array() ) {
	...
	foreach ( $results as $result ) {
		...
		$str = "<a href=\"".$result[$_cols["href"]]."\" target=_blank class=\"external\" rel=\"nofollow\"";
