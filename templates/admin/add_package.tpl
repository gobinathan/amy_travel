...
<div id="frm">
<label>From Date:</label><input type="text" value="{if $listing.start_date != "0"}{$listing.start_date|date_format:"%d/%m/%y"}{else}{$smarty.post.start_date}{/if}" name="start_date" id="start_date" size="8" onMouseOver="showhint('{#hint_package_start_date#}', this, event, '150px')" /><img src="{$BASE_URL}/images/calendar.gif" id="f_trigger_s" style="cursor: pointer; border: 1px solid red;" title="Date selector" onmouseover="this.style.background='red';" onmouseout="this.style.background=''" /><br/>
{literal}
...
{/literal}
<label>Price (<b>{$listing.currency}</b>):</label><input type="text" name="base_price" size="4" value="{$smarty.post.base_price}"></label><br/><label>{#per#} </label><select name="price_period" style="width:70px;"><option value="1" {if $pack.price_period eq "1"}selected{/if}>{#day#}</option><option value="7" {if $pack.price_period eq "7"}selected{/if}>{#week#}</option><option value="30" {if $pack.price_period eq "30"}selected{/if}>{#month#}</option><option value="365" {if $pack.price_period eq "365"}selected{/if}>{#year#}</option></select>
<br/>
<fieldset style="width:600px;text-align:center;"><legend>Discounts</legend>
<fieldset style="float:left;"><legend>People</legend>
<label>Count:</label><input type="text" name="people_count" size="2" value="{$smarty.post.people_count}"/><br/>
<label>Discount:</label><input type="text" name="people_discount" size="2"  value="{$smarty.post.people_discount}"/> <b>%</b><br/>
</fieldset>
<fieldset style="float:left;"><legend>Rooms</legend>
<label>Count:</label><input type="text" name="rooms_count" size="2"  value="{$smarty.post.rooms_count}"/><br/>
<label>Discount:</label><input type="text" name="rooms_discount" size="2"  value="{$smarty.post.rooms_discount}"/> <b>%</b><br/>
</fieldset>
<fieldset style="float:left;"><legend>Kids</legend>
<label>Count:</label><input type="text" name="kids_count" size="2"  value="{$smarty.post.kids_count}"/><br/>
<label>Discount:</label><input type="text" name="kids_discount" size="2"  value="{$smarty.post.kids_discount}"/> <b>%</b><br/>
</fieldset>
</fieldset>
