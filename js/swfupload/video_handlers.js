...
function uploadProgress(file, bytesLoaded, bytesTotal) {
	...
	try {
		...
		if (percent === 100) {
			progress.setStatus("Converting video...");
...
//		var uSpeed = Math.floor(roundNumber(((bytesLoaded/uTime)/1024),2));
		var uSpeed = Math.floor(roundNumber((bytesLoaded * 8 ) / (uTime / 1000),2));
...
//		addImage(serverData);
		addSpan(serverData);
		var progress = new FileProgress(file,  this.customSettings.upload_target);
		progress.setStatus("Video Converted.");
...
function uploadComplete(file) {
	try {
		...
		if (this.getStats().files_queued > 0) {
			...
			progress.setStatus("All videos received.");			
			progress.toggleCancel(false);
		}
	} catch (ex) {
		this.debug(ex);
...
// create span element
	...
	document.getElementById("thumbnails").appendChild(sp);
}

function addImage(video_id) {
	var src = "video_thumbnail.php?id=" + video_id;
	var alink = document.createElement("A");
	alink.href = "videos.php?play=" + video_id;
	alink.target = "_blank";
	alink.title = "Play";
