...
$getlisting=mysql_query("SELECT * from `listings` LEFT JOIN `listings_text` ON (listings.listing_id=listings_text.listing_id) WHERE `uri`='$request[1]' AND (`lang`='$default_lang' OR `lang`='$language') ORDER BY FIELD(lang,'$language','$default_lang')");
...
$default_listing_currency=@mysql_result(mysql_query("SELECT `currency` from `listings` WHERE `listing_id`='$request[1]'"),0);
$packages=array();
...
if ($request[2]=="contact") {
	...
	if (isset($_POST['interested_in'])) {
		...
		if (!is_email($email)) { $error[]=$lang_errors['invalid_from_email']; }
		if (md5($number) !== $_SESSION['image_random_value']) { $error[]=$lang_errors['wrong_captcha']; }
		// If no errors...continue
		if (count($error)=="0")	{
		...
		// Send E-Mail
		mail($listing[member][email], $subject, $email_message, $headers);
